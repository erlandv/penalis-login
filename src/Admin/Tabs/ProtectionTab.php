...
final class ProtectionTab {
	...
	public function renderIpSection(): void {
		...
		<table class="form-table" role="presentation"><tbody>
			...
			<p><?php esc_html_e( 'If your site is behind a reverse proxy or Cloudflare, configure trusted proxy IPs or CIDR ranges here so rate limiting and IP access control use the real visitor IP instead of the proxy IP.', 'penalis-login' ); ?></p>
		<table class="form-table" role="presentation"><tbody>
			<tr>
				...
				<tr>
					<th scope="row"><label for="penalis_trusted_proxies"><?php esc_html_e( 'Proxy IPs / CIDR Ranges', 'penalis-login' ); ?></label></th>
	...
	private function renderTrustedProxiesEnabledField(): void {
		...
		<label for="penalis_trusted_proxies_enabled">
			<input type="checkbox" id="penalis_trusted_proxies_enabled"
				...
				value="1" <?php checked( (bool) $val ); ?> />
				<?php esc_html_e( 'Trust proxy headers (X-Forwarded-For, CF-Connecting-IP, X-Real-IP) from the IPs or CIDR ranges listed below', 'penalis-login' ); ?>
		</label>
		<div class="penalis-field-note penalis-field-note-warning" style="margin-top:8px; max-width:680px;">
			<span><span class="dashicons dashicons-warning" aria-hidden="true"></span> <?php esc_html_e( 'Only enable this if your site is behind a reverse proxy. Enabling it without adding the correct proxy IPs or CIDR ranges below will make IP-based security features unreliable.', 'penalis-login' ); ?></span>
		</div>
		<?php
	}
	...
	private function renderTrustedProxiesField(): void {
		...
		<textarea
			...
			rows="4"
				class="large-text code penalis-ip-textarea"
				placeholder="103.21.244.0/22 # Cloudflare&#10;103.22.200.0/22 # Cloudflare&#10;127.0.0.1 # local Nginx"
			spellcheck="false"
		><?php echo esc_textarea( $val ); ?></textarea>
			<p class="description">
				<?php esc_html_e( 'One IP address or CIDR range per line. Inline comments supported. Only requests arriving from these IPs or ranges will have their proxy headers trusted.', 'penalis-login' ); ?>
			</p>
		<?php
	}
